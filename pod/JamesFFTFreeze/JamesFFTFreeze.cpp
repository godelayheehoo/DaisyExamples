...
const float METRO_FREQ = 2.0f; // Default metro frequency

//project related constants
bool freeze_active = false;

static FFT fft_impl;                    // ShyFFT or arm rfft instance
static STFT stft;
static float* fft_buffer;               // forward complex buffer (size dependent)
static float* ifft_buffer;              // inverse buffer
static float* window_lut;               // hann window, length fft_size
static short* stft_proc_scratch;        // STFT wants a short* scratch (see Init)
static const size_t FFT_SIZE = 1024;
static const size_t HOP_SIZE = FFT_SIZE / 4 ;
bool freeze_active = false;
// Buffer to store frozen FFT spectrum
static float frozen_fft_buffer[FFT_SIZE];
